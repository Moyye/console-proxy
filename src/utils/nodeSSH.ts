...
const DEFAULT_VALIDATE = (path: string) =>
  !fsPath.basename(path).startsWith('.');
const DEFAULT_TICK = () => {
  /* No Op */
};
...
export class NodeSSH {
  ...
  public async connect(givenConfig: Config): Promise<this> {
    ...
    if (config.host != null) {
      ...
      throw new invariant.AssertionError({
      ...
      if (
        ...
        try {
          ...
          if (err != null && err.code === 'ENOENT') {
            throw new invariant.AssertionError({
              message: 'config.privateKey does not exist at given fs path',
            });
          }
          throw err;
        }
        ...
        sftp.fastPut(
          ...
          (err) => {
            ...
            if (err.message === 'No such file' && retry) {
              resolve(
                this.mkdir(fsPath.dirname(remoteFile), 'sftp', sftp).then(() =>
                  putFile(false),
                ),
              );
            } else {
  ...
  async putDirectory(
    ...
    const files = scanned.files.map((item) =>
      fsPath.relative(localDirectory, item),
    );
      ...
      await new Promise((resolve, reject) => {
        ...
        directories.forEach((directory) => {
          queue
            .add(async () => {
              await this.mkdir(
                fsPath.join(remoteDirectory, directory),
                'sftp',
                sftp,
              );
            })
  ...
  async getDirectory(
    ...
    const files = scanned.files.map((item) =>
      fsPath.relative(remoteDirectory, item),
    );
    const directories = scanned.directories.map((item) =>
      fsPath.relative(remoteDirectory, item),
    );
